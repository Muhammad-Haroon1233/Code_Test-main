...
class BookingController extends Controller
{
    ...
    /**
     * Store a newly created job email in storage.
     *
     * @param Request $request
     * @return mixed
     */
    ...
    public function distanceFeed(Request $request)
    {
        ...
        if (isset($data['distance']) && $data['distance'] != "") {
            $affectedRows = Distance::where('job_id', '=', $data['jobid'])->update(['distance' => $data['distance'], 'time' => $data['time']]);
        }

        // Update admin comments and other job details if provided
        if (isset($data['admincomment']) || isset($data['session_time']) || isset($data['flagged']) || isset($data['manually_handled']) || isset($data['by_admin'])) {
            $affectedRows1 = Job::where('id', '=', $data['jobid'])->update([
